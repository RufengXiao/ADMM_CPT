...
for dataname = ["daily_used","SP500_2"]%["daily_used","SP500_2","rand"] % options:
    ...
    for iter_num = 1:expe_num
        ...
        t2 = clock;
        filename = "./res_w/w_"+dataname+"_"+num2str(period)+".mat";
    ...
    savefile = './res/ADMM_PAV_'+dataname+'.mat';
    ...
    for i = 1:expe_num
        ...
        size(R)
        t1 = clock;
        [an,bn] = coefficients_generating2(N,args.delta_pos,args.delta_neg);
        A = -eye(m,m);
        b = zeros(m,1);
        % %  drop the x >= 0 constr
        % A = [];
    ...
    savefile = './res/fmincon_'+dataname+'.mat';
